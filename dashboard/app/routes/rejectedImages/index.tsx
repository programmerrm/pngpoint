...
import { useGetRejectedImagesQuery } from "~/redux/features/images/rejectedImagesApi";
...
export default function Images() {
    ...
    const { data, isLoading, isError, isSuccess } = useGetRejectedImagesQuery(undefined, { refetchOnMountOrArgChange: true });
    ...
    if (isLoading) {
        return <div className="text-center py-10 text-gray-600">Loading images...</div>;
    }
    if (isError) {
        return <div className="text-center py-10 text-red-600">Failed to load images.</div>;
    }
    ...
    return (
        <div className="flex flex-col flex-wrap px-10 pb-3.5 h-[82%] relative">
            <div className="flex flex-row flex-wrap items-center justify-between h-[7%]">
                <h4 className="text-base font-medium">Rejected images : {count}</h4>
            </div>
